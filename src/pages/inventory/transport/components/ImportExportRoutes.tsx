...
import { importTransportRoutes, exportTransportRoutes } from '@/utils/transportRoutesImportExport';

type Format = 'xlsx' | 'csv' | 'json';

interface ImportExportRoutesProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const ImportExportRoutes: React.FC<ImportExportRoutesProps> = ({ isOpen, onClose }) => {
  const fileRef = useRef<HTMLInputElement>(null);
  const { toast } = useToast();

  // Import state (hidden in UI for focused testing)
  ...
  // Import handler intentionally not rendered to keep only one Export button visible during debugging
  const handleImport = async () => {
    try {
      ...
      toast({
        ...
        description: `${result.validCount} rows imported. ${result.invalidCount} invalid rows${result.invalidCount ? ' (see console)' : ''}.`,
      });
