...
import { fetchActiveAgentSessions, fetchAgentLoginRecords } from '@/services/agentLoginRecordService';
import type { AgentActiveSession, AgentLoginRecord, AgentLoginFilters } from '@/services/agentLoginRecordService';
...
const AgentLoginReport: React.FC = () => {
  // Tracking is always on
  const [trackingEnabled] = useState<boolean>(true);
  const [activeSessions, setActiveSessions] = useState<AgentActiveSession[]>([]);
  const [loginRecords, setLoginRecords] = useState<AgentLoginRecord[]>([]);
  ...
  const STORAGE_KEY = 'agentLoginReport:filters';

  const loadData = async () => {
    setLoading(true);
    try {
      // Build server-side filters (omit "all" and "Unknown")
      const filters: AgentLoginFilters = {
      ...
      const sessions = await fetchActiveAgentSessions(filters);
      const records = await fetchAgentLoginRecords(1000, filters);
      // Sort for display
      setActiveSessions(
      ...
      console.warn('Agent login report load error:', e);
    } finally {
      setLoading(false);
    }
  };

  // Load persisted filters, then initial fetch
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    const channel = supabase.channel('agent-login-report')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_login_records' }, () => {
        loadData();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_active_sessions' }, () => {
        loadData();
      })
      .subscribe();
  ...
  return (
    <PageLayout title="Agent Login Report" description="View active agent sessions and recent login history">
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-bold tracking-tight">Agent Login Report</h2>
          ...
          <Card className="bg-white dark:bg-gray-800 shadow-sm border-gray-200 dark:border-gray-700">
            ...
            <CardContent>
              {totalActive === 0 ? (
                <div className="text-sm text-muted-foreground py-6">No active agent sessions.</div>
              ) : (
                <Table compact hoverable>
                  ...
                  <TableBody>
                    {activeSessions.map((s) => (
                      <TableRow key={`${s.agentId}-${s.loginTime}`} highlight>
                        <TableCell truncate>{s.agentName}</TableCell>
                ...
                <Table compact hoverable>
                  ...
                  <TableBody>
                    {filteredLoginRecords.map((r) => (
                      <TableRow key={r.id}>
                        <TableCell truncate>{r.agentName}</TableCell>
