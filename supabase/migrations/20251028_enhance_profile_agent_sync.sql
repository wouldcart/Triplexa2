-- Enhance profile→agent sync to include source fields and creator info
-- Only creates/updates agents when profile.role = 'agent'

BEGIN;

-- Role-aware insert: pull source fields from user_profile_data or auth.users metadata
...
BEGIN
  ...
  -- Prefer source info from user_profile_data (if available)
  ...
  -- Fallback: read from auth.users.raw_user_meta_data
  ...
  -- Sensible defaults for /signup/agent flow
  ...
  INSERT INTO public.agents AS a (
    ...
    NEW.id, NEW.id, NEW.name, NEW.email, v_agency_name, v_business_phone, v_country, v_city,
    COALESCE(NEW.status, 'active'), v_source_type, v_source_details, v_created_by, v_created_by_staff, NOW(), NOW()
  )
...
-- Role-aware update: keep in sync and preserve existing non-empty values
...
BEGIN
  ...
  SET
    ...
    agency_name = COALESCE(v_agency_name, a.agency_name),
    business_phone = COALESCE(v_business_phone, a.business_phone),
    country = COALESCE(NEW.country, a.country),
    city = COALESCE(NEW.city, a.city),
    source_type = COALESCE(NULLIF(v_source_type, ''), a.source_type),
    source_details = COALESCE(NULLIF(v_source_details, ''), a.source_details),
    created_by = COALESCE(a.created_by, v_created_by),
    created_by_staff = COALESCE(a.created_by_staff, NULLIF(v_created_by_staff, '')),
    updated_at = NOW()
