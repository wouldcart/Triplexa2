-- Fix user metadata extraction for profile creation
-- This migration addresses the issue where signup form data (name, phone, company_name) 
-- is not being extracted properly because functions are looking for raw_user_meta_data 
-- instead of user_metadata where the data is actually stored.
...
-- 2. Create updated handle_new_user function that uses user_metadata
...
BEGIN
  -- Extract metadata from user_metadata (where signup data is actually stored)
  -- with fallbacks to raw_user_meta_data for backward compatibility
  user_name := COALESCE(
  ...
  user_status := COALESCE(
    NULLIF(NEW.user_metadata->>'status', ''),
...
-- 4. Update get_or_create_profile_for_current_user function to use user_metadata
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Updated trigger function that extracts user metadata from both user_metadata and raw_user_meta_data with proper fallbacks for profile creation';
COMMENT ON FUNCTION public.get_or_create_profile_for_current_user() IS 'Updated function that extracts user metadata from both user_metadata and raw_user_meta_data with proper fallbacks for profile creation';
